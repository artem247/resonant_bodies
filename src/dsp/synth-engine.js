...
export class ResonantBodiesSynth {
  constructor(sampleRate) {
    ...
    /* ----------  LFOs ------------- */
    const groupSize = 2; // 1 LFO per 2 resonators
    const lfoCount = Math.ceil(Config.audio.maxResonators / groupSize);
    this.lfos = Array.from(
      { length: lfoCount },
      (_, k) =>
        new LFO(
          0.05 + Math.random() * 0.4, // 0.05 – 0.45 Hz
          0.01 + Math.random() * 0.01, // 0.01 – 0.02 depth
          sampleRate,
        ),
    );
